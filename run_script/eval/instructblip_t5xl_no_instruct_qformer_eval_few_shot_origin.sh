...
datasets=("flickr" "gqa" "miniimage" "nlvr2" "okvqa" "vqa" "wikiart")  # 添加其他数据集名称到这个数组

for dataset in "${datasets[@]}"; do
  echo "Running evaluation for dataset: $dataset" 
    EXPERIMENT_NAME=instruct_BLIP2_deepSpeed_t5xl_no_instructqformer_few_origin/eval_${dataset} 
    python run.py \
