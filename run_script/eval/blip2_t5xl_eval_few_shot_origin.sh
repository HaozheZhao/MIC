...
model_type=blip2
datasets=("flickr" "gqa" "miniimage" "nlvr2" "okvqa" "vqa" "wikiart" "vcr" "msrvttqa" "msrvtt")  # 添加其他数据集名称到这个数组

for dataset in "${datasets[@]}"; do
  echo "Running evaluation for dataset: $dataset" 
    EXPERIMENT_NAME=BLIP2_t5xl_few_shot_origin/eval_${dataset} 
    python run.py \
    ...
    --run_name blip2-flan-t5-xl-few_shot_origin_${dataset}  \
    --only_evaluate True \
