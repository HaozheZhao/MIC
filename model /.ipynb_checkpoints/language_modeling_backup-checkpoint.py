...
class BertPromptForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.verbalizer_id = config.verbalizer_id
        self.prompt_operation = config.prompt_operation
        ...
        self.multiple_choice = config.multiple_choice
        # self.prefix_tokens = torch.arange(self.pre_seq_len).long()
        # self.prefix_encoder = torch.nn.Embedding(self.pre_seq_len, config.hidden_size)
        self.prompt_encoder = PromptEncoder(config.tokenizer, self.bert, self.hidden_size, self.pre_seq_len, self.prompt_operation, config.hidden_dropout_prob)

    def get_output_embeddings(self):
        return self.cls.predictions.decoder

    def set_output_embeddings(self, new_embeddings):
        self.cls.predictions.decoder = new_embedding, self.pre_seq_len
    ...
    def forward(
        ...
        if self.prompt_operation in ["max", "mean", "sum", "attention"]:
            prompts = torch.ones(batch_size, self.pre_seq_len, self.hidden_size).to(self.device)
            for batch_id in range(batch_size):
                for seq_id in range(self.pre_seq_len):
                    prompts[batch_id, seq_id, :] = self.prompt_encoder(sentences_ids[batch_id][sentences_ids[batch_id] != self.pad_token_id])[:] # 这一步已经把pad_token_id排除了。
        elif self.prompt_operation in ["none"]:
            prompts = torch.ones(batch_size, self.pre_seq_len, self.hidden_size).to(self.device)
            prompts_replace = self.prompt_encoder()
            for batch_id in range(batch_size):
                for seq_id in range(self.pre_seq_len):
                    prompts[batch_id, seq_id, :] = prompts_replace if self.pre_seq_len == 1 else prompts_replace[seq_id, :]
        else:
            raise NotImplementedError("The prompt_operation for {} has not been defined.".format(self.prompt_operation))

        attention_mask = input_ids != self.pad_token_id # batch_size * seq_length
        ...
        input_for_embedding[(input_ids == self.pseudo_token_id)] = self.unk_token_id # 转化[PROMPT]_id为[UNK]_id
        inputs_embeds = self.embeddings(input_for_embedding) # 转化token_id [batch_size, seq_len]为 embedding [batch_size, seq_len, embedding_dim]

        if self.prompt_type == "none":
            pass
        else:
            blocked_indices = (input_ids == self.pseudo_token_id).nonzero().reshape((batch_size, self.pre_seq_len, -1))[:, :, 1]  # 把[PROMPT]标记为1，反转，[PROMPT]标记为0，返回的是[PROMPT]的索引
            for batch_id in range(batch_size):
                for seq_id in range(self.pre_seq_len):
                    inputs_embeds[batch_id, blocked_indices[batch_id, seq_id], :] = prompts[batch_id, seq_id, :]

        # get the label input
        # labels = torch.empty_like(batch_queries).fill_(-100).long().to(self.device)  # batch_size * max_sql_length
        label_mask = (input_ids == self.mask_token_id).nonzero().reshape(batch_size, num_mask_token, -1)[:, :, -1].to(self.device) # batch_size * num_mask
        # labels = torch.empty_like(batch_queries).fill_(-100).long().to(self.device) # batch_size * max_sql_length
        ...
        # label_token_id
        # label_ids_all = torch.LongTensor([[self.tokenizer.encode(l)[1:-1] for l in self.labels] for _ in range(batch_size)]).squeeze(2).to(self.device) # batch_size * num_labels
