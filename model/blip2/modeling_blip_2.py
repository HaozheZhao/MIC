...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel):
    ...
    def forward(
        ...
        for index,pixel in enumerate(pixel_values):
            ...
            inputs_embeds = self.language_model.get_input_embeddings()(input_ids[index])
            if attention_mask is None:
                attention_mask = torch.ones_like(input_ids)
            expected_device = language_model_attention_mask.device
            attention_mask.to(expected_device)

            emd=[inputs_embeds[:image_embeds_index[0]]]
            ...
            lm_input_size.append(32*pictures.shape[0]+torch.nonzero(attention_mask[index])[-1].item()+1-pictures.shape[0])
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        for index,pixel in enumerate(pixel_values):
            ...
            image_embeds = self.vision_model(pictures, return_dict=True).last_hidden_state
            image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device)
            ...
            if attention_mask is None:
                attention_mask = torch.ones_like(input_ids[index])
            inputs_embeds = self.language_model.get_input_embeddings()(input_ids[index])
            if not caption_flag:
                emd=[inputs_embeds[:image_embeds_index[0]]]
                mask = [attention_mask[index][:image_embeds_index[0]]]
                ...
                lm_input_size.append(32*pictures.shape[0]+attention_mask.shape[0])
        # min_padding_size = max(lm_input_size) if max(lm_input_size) <512 else 512
        min_padding_size = 512
        inputs_embeds = torch.stack([ lm_input[:min_padding_size] for lm_input in lm_inputs])
        attention_mask = torch.stack([ lm_mask[:min_padding_size] for lm_mask in lm_masks])
